...
namespace Synergit.Maui.Camera.Platforms.Android;
...
internal class MauiCameraView: GridLayout
{
            ...
            if (await CameraView.RequestPermissions())
            {
                ...
                if (cameraView.Camera != null)
                {
                    try
                    {
                        ...
                        videoSize = ChooseMinVideoSize(map.GetOutputSizes(Class.FromType(typeof(ImageReader))));
    ...
    private void AdjustAspectRatio(int videoWidth, int videoHeight)
    {
        ...
        RectF previewRectF = new(0, 0, Width, Height);
        float centerX = previewRectF.CenterX();
        float centerY = previewRectF.CenterY();
        ...
        if (rotation == SurfaceOrientation.Rotation90 || rotation == SurfaceOrientation.Rotation270)
        {
            ...
            RectF textureRectF = new(0, 0, videoHeight, videoWidth);
            textureRectF.Offset(centerX - textureRectF.CenterX(), centerY - textureRectF.CenterY());
            txform.SetRectToRect(previewRectF, textureRectF, Matrix.ScaleToFit.Fill);
    ...
    {
        if (previewSession != null && previewBuilder != null && cameraView != null && cameraView.Camera != null)
        {
            try
            {
                if (cameraView.Camera.HasFlashUnit)
    ...
    private sealed class MyCameraStateCallback : CameraDevice.StateCallback
    {
    ...
    private sealed class PreviewCaptureStateCallback : CameraCaptureSession.StateCallback
    {
        private readonly MauiCameraView cameraView;

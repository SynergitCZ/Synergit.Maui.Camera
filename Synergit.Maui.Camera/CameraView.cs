...
public class CameraView : View, ICameraView
{
...
#if ANDROID
        lumSource = new Synergit.Maui.Camera.Platforms.Android.BitmapLuminanceSource(data);
#elif IOS || MACCATALYST
        lumSource = new Synergit.Maui.Camera.ZXingHelper.RGBLuminanceSource(data);
#elif WINDOWS
        lumSource = new Synergit.Maui.Camera.Platforms.Windows.SoftwareBitmapLuminanceSource(data);
#endif
    ...
    public async Task<CameraResult> StartCameraAsync(Size Resolution = default)
    {
        ...
        if (Camera != null)
        {
            ...
            if (Resolution.Width != 0 && Resolution.Height != 0 && !Camera.AvailableResolutions.Exists(r => r.Width == Resolution.Width && r.Height == Resolution.Height))
